...
## Name the rule, when the project has one

If the codebase carries a written rule set with ids - an ArchUnit suite, a lint
config, a conventions doc, a CONTRIBUTING - say which rule a finding breaks and use
that id. It goes into `detail`, never into `evidence`, which stays code:

> `STR-3`: Der Controller greift direkt auf das Repository zu, die Service-Schicht
> wird auf diesem Pfad übersprungen.

Two things that buys: the finding and the project's own review vocabulary become
greppable as one, and an id is a claim you have to be able to back. It stays a
pointer, not a review - the scan sees declarations and edges, whether the rule is
really broken is decided with the file open.
...
seam it would split along, and in JPA an embeddable is what they split into.
...
kept as `String`. Every value check is a string comparison. An `@Enumerated` left on
the `ORDINAL` default is the worse half of the same finding - it stores a position,
so inserting a value in the middle of the enum silently rewrites what every existing
row means. The model shows the column type either way.
