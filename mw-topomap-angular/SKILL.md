...
throwaway generator in the scratchpad, project by project, and let it dump the json
with `json.dump(model, fh, ensure_ascii=False, indent=1)` - **`ensure_ascii=False` is
not optional**, see the language rule below.
