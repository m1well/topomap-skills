...
```jsonc
{
  "meta": {
    ...
    "stereotypes": {                 // optional, adds a stereotype the preset has not
      "worker": { "label": "worker", "rank": 5,
                  "icon": "bolt", "color": "var(--mw-warning-color)" }
    },
  ...
  "nodes": [                          // the classes
    {
      ...
      "stereotype": "api",            // see table below, drives colour, icon and column
      "tags": ["cached"],             // optional extra badges
...
- The same edge may appear in several flows and more than once in one flow.
